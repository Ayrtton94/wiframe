...
class ReportController extends Controller
{
    ...
    public function salidas(Request $request)
    {
        ...
        $query = DB::table('sales as s')
            ...
            */
            ...
            ->select([
                ...
                's.sold_by as responsible_id',
                'si.store_id as product_id',
        ...
        */
        ...
        /*
         |--------------------------------------------------------------------------
         | PAGINACIÓN POR SALIDA
         |--------------------------------------------------------------------------
         | Cada salida puede tener varios sale_items. Paginar directamente los
         | items puede partir una salida entre dos páginas. Primero obtenemos
         | todos los items filtrados y luego paginamos por ID de salida para que
         | cada salida llegue completa al frontend.
         */
        ...
        $pageRows = $allRows
            ->filter(fn ($row) => $pageSaleIds->contains($row->id))
            ->values();
    ...
    public function movimientoProductos(Request $request)
    {
        ...
        */
        ...
        $items = $movementKeys->map(
            function ($row) use ($from, $to) {
                ...
                */
                ...
                |--------------------------------------------------------------------------
                |
                | Ahora se obtiene directamente desde stock_initials.
                | NO se reconstruye con el stock actual.
                |
                */

                $stockInitial = StockInitial::query()
                    ->where(
                        'store_id',
                        $row->product_id
                    )
                ...
                $saldoInicial = $stockInitial
                    ? (
                        $row->unidad === 'ROLLOS'
                            ? (float) $stockInitial->kilos_initial
                            : (float) $stockInitial->metros_initial
                    )
                    : 0.0;
    ...
    public function inventario(Request $request)
    {
        ...
        $warehouses = $this->reportWarehouses(
            $assignedWarehouseIds
        );
        ...
        $query = DB::table('warehouse_stocks as ws')
            ...
            // Si quieres mostrar también productos desactivados,
            // NO colocar p.is_active aquí.
            ->where(
                'w.is_active',
                true
            )
   ...
   public function exportSalidas(Request $request)
    {
        ...
        return Excel::download(
            new \App\Exports\SalidasExport($filters),
